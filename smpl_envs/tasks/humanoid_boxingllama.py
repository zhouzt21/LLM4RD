...
class HumanoidBoxing(humanoid_amp_task.HumanoidAMPTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.self_root_state, self.self_body_pos, self.oppo_root_state, self.oppo_body_pos, self.oppo_dof_pos, self.self_contact_norm, self.oppo_contact_norm, self.hand_ids)
        self.extras['gpt_reward'] = self.rew_buf.mean()
        for rew_state in self.rew_dict: self.extras[rew_state] = self.rew_dict[rew_state].mean()  # 原本是私有的
        for i in range(self.num_agents):
